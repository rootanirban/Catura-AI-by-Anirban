...
@app.get("/ping")
def ping():
    return {"status": "ok", "timestamp": datetime.utcnow().isoformat(), "version": "0.0.30"}
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "version": "0.0.30", "timestamp": datetime.utcnow().isoformat()}
...
async def chat_post(request: Request):
    try:
        ...
        if model_key in GEMMA_GOOGLE_MODELS:
            google_model_id = GEMMA_GOOGLE_MODELS[model_key]
            print(f"🟢 [Gemma GET] Routing '{model_key}' → Google AI Studio model: {google_model_id}")
